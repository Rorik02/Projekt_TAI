...
import Modal from "../components/Modal/Modal"; // Upewnij się, że ta ścieżka jest poprawna względem struktury folderów
...
const Dashboard = () => {
  ...
  // Formularz Restauracji (Teraz z kategorią i adresem)
  ...
  // --- OBSŁUGA RESTAURACJI (DODAWANIE I USUWANIE) ---

  const handleAddRestaurant = async (e) => {
    e.preventDefault();
    try {
        // Backend oczekuje tablicy stringów w 'cuisines'
        const payload = {
            name: restForm.name,
        ...
        // Reset formularza
        setRestForm({ name: "", category: CATEGORIES[0], address: "", rating: 5.0 });
        setIsRestModalOpen(false);
        handleSelectRestaurant(newRest);
    } catch (err) {
        console.error(err);
        alert("Błąd dodawania restauracji. Upewnij się, że zaktualizowałeś bazę danych (nowe pole address).");
    }
  };

  const handleRemoveRestaurant = async (e, restaurantId) => {
    e.stopPropagation(); // Zapobiega wyborowi restauracji przy kliknięciu w kosz
    if (!window.confirm("Czy na pewno chcesz usunąć tę restaurację? Usunie to również jej menu.")) return;
    ...
    try {
        ...
        // Jeśli usunięto aktualnie wybraną, czyścimy widok menu
  ...
  // --- OBSŁUGA PRODUKTÓW (DODAWANIE I USUWANIE) ---
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-white p-6">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-200px)] min-h-[500px]">
            ...
            {restaurants.length === 0 ? (
                ...
                >
                    ...
                    <div className="flex items-center ml-2">
                        {/* Przycisk usuwania (widoczny po najechaniu - group-hover) */}
                        <button
      ...
      <Modal isOpen={isRestModalOpen} onClose={() => setIsRestModalOpen(false)} title="Dodaj nową restaurację">
        <form onSubmit={handleAddRestaurant} className="space-y-4">
            ...
            {/* Adres */}
            <div>
                <label className="block text-sm font-medium mb-1 dark:text-gray-300">Adres</label>
                <input 
                    type="text" 
                    className="w-full p-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-purple-500 outline-none dark:text-white"
